...
cells = np.zeros((ROWS,COLS), dtype=np.int0)
...
def clear_buttons(array):
    #global buttons
    for row in range(ROWS):
        for col in range(COLS):
            array[row][col]['bg'] = '#F0F0F0'
...
clear_button = Button(top_menu, text="Clear", font=('Arial',14), command=lambda: clear_buttons(buttons))
