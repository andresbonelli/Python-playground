...
def memoizationFib(n):
    ...
    However, recursion limit can happen if handling larger numbers.
    :param n:
    :return:
    """
    try:
...
def noMemoFib(n):
    ...
    It has the same time complexity as the tabulation and memoization methods, O(n) but
    has constant space complexity, O(1). (only stores last 2 fib numbers instead of complete sequence)
    :param n:
    :return:
    """
    if n < 2: return n
